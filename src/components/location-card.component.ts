    ...
    <div class="mt-4 mb-4 rounded-3xl overflow-hidden border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-2xl animate-slide-up max-w-sm group/card">
      ...
      <div class="p-5">
        <div class="flex items-center gap-4 mb-5">
          <div class="flex-1">
            <p class="text-[10px] uppercase tracking-[0.15em] text-slate-400 font-bold mb-1">{{ translationService.t().currentLocation }}</p>
        ...
        <div class="grid grid-cols-2 gap-3 mb-5">
          <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50">
            <p class="text-[9px] uppercase text-slate-400 font-bold mb-1">{{ translationService.t().latitude }}</p>
            <p class="text-xs font-mono font-bold text-slate-700 dark:text-slate-200">{{ location().lat.toFixed(4) }}</p>
          </div>
          <div class="p-3 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50">
            <p class="text-[9px] uppercase text-slate-400 font-bold mb-1">{{ translationService.t().longitude }}</p>
...
export class LocationCardComponent {
  location = input.required<{lat: number, lng: number, label?: string}>();
  translationService = inject(TranslationService);
  private sanitizer = inject(DomSanitizer);

  getMapUrl(lat: number, lng: number): SafeResourceUrl {
    const url = `https://www.google.com/maps/embed/v1/place?key=YOUR_GOOGLE_MAPS_API_KEY&q=${lat},${lng}&zoom=15`;
    return this.sanitizer.bypassSecurityTrustResourceUrl(url);
  }

  openInMaps(lat: number, lng: number, external = true) {
    const url = external 
