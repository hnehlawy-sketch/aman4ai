...
@Component({
  ...
  template: `
    <div class="w-full mb-2 group flex" 
         *ngIf="message().text || message().generatedImages?.length || message().generatedFile || message().location || message().route || message().fileData || isEditing()"
...
export class MessageBubbleComponent {
  ...
  async downloadGeneratedFile(file: NonNullable<ChatMessage['generatedFile']>) {
    if (file.url) {
      // Use the pre-generated URL if available
      const link = document.createElement('a');
      link.href = file.url;
      link.download = file.filename;
      link.click();
    } else {
      // Fallback to generating on the fly if no URL
      if (file.type === 'pdf') {
        const blob = await this.documentGeneratorService.generatePDF(file.content, file.filename);
        this.documentGeneratorService.downloadFile(blob, file.filename);
      } else if (file.type === 'docx') {
        const blob = await this.documentGeneratorService.generateDocx(file.content, file.filename);
        this.documentGeneratorService.downloadFile(blob, file.filename);
      } else {
