    ...
    <div class="fixed inset-0 z-[60] flex flex-col bg-slate-50 dark:bg-slate-950 overflow-y-auto animate-[fadeIn_0.2s_ease-out] font-sans"
      ...
      <div class="sticky top-0 z-20 flex items-center justify-between p-3 sm:p-4 sm:px-8 bg-white/80 dark:bg-slate-950/80 backdrop-blur-xl border-b border-slate-200 dark:border-slate-800">
        <div class="flex items-center gap-2 sm:gap-3">
          ...
          <div>
            <h1 class="text-base sm:text-lg font-black tracking-tight leading-none">{{ t().upgradeToPro }}</h1>
            <p class="text-[8px] sm:text-[10px] font-bold text-slate-500 uppercase tracking-widest mt-0.5 sm:mt-1">{{ t().premiumExperience }}</p>
          </div>
        </div>
        ...
        @if (!selectedPlanForPayment()) {
          ...
          <div class="flex-1 p-4 sm:p-8 max-w-6xl mx-auto w-full flex flex-col justify-center">
            <div class="text-center mb-8 sm:mb-12 animate-[slideUp_0.3s_ease-out]">
              ...
              <h2 class="text-3xl sm:text-5xl font-black mb-3 sm:mb-4 tracking-tight leading-tight">{{ t().planTitle }}</h2>
              <p class="text-slate-500 dark:text-slate-400 text-sm sm:text-lg max-w-2xl mx-auto px-4">{{ t().planDesc }}</p>
            ...
            <div class="grid grid-cols-2 gap-3 sm:gap-8 max-w-5xl mx-auto w-full animate-[slideUp_0.4s_ease-out]">
              ...
              <div class="group relative p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border transition-all duration-500 hover:shadow-2xl hover:-translate-y-1 flex flex-col overflow-hidden"
                ...
                <div class="mb-6 sm:mb-8">
                  ...
                  <p class="text-xs sm:text-sm text-slate-500 mb-4 sm:mb-6">{{ t().proPlanDesc }}</p>
                  <div class="flex items-baseline gap-1.5 sm:gap-2">
                    <span class="text-4xl sm:text-5xl font-black tracking-tighter">{{ getPrice('pro', 'usd') }}$</span>
                    <span class="text-slate-500 text-xs sm:text-sm font-bold">/ {{ billingCycle() === 'monthly' ? t().month : t().year }}</span>
                  </div>
                  <div class="text-xs sm:text-sm font-bold text-blue-600 dark:text-blue-400 mt-1.5 sm:mt-2">{{ t().or }} {{ getPrice('pro', 'syp') }} {{ t().syp }}</div>
                </div>
              ...
              <div class="group relative p-6 sm:p-8 rounded-[2rem] sm:rounded-[2.5rem] border-2 border-blue-600 shadow-xl sm:shadow-2xl shadow-blue-500/10 sm:shadow-blue-500/20 transition-all duration-500 hover:-translate-y-1 flex flex-col overflow-hidden"
                ...
                <div class="mb-6 sm:mb-8">
                  ...
                  <p class="text-xs sm:text-sm text-slate-500 mb-4 sm:mb-6">{{ t().premiumPlanDesc }}</p>
                  <div class="flex items-baseline gap-1.5 sm:gap-2">
                    <span class="text-4xl sm:text-5xl font-black tracking-tighter">{{ getPrice('premium', 'usd') }}$</span>
                    <span class="text-slate-500 text-xs sm:text-sm font-bold">/ {{ billingCycle() === 'monthly' ? t().month : t().year }}</span>
                  </div>
                  <div class="text-xs sm:text-sm font-bold text-blue-600 dark:text-blue-400 mt-1.5 sm:mt-2">{{ t().or }} {{ getPrice('premium', 'syp') }} {{ t().syp }}</div>
                </div>
          ...
          <div class="flex-1 flex flex-col lg:flex-row animate-[fadeIn_0.4s_ease-out]">
            ...
            <div class="lg:hidden bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 p-4 sticky top-0 z-10 shadow-sm">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-3">
                  ...
                  <div>
                    <div class="text-sm font-black capitalize">{{ selectedPlanForPayment() }} {{ t().plan }}</div>
                    <div class="text-[10px] font-bold text-blue-600 dark:text-blue-400">{{ getPrice(selectedPlanForPayment()!, 'usd') }}$ / {{ getPrice(selectedPlanForPayment()!, 'syp') }} {{ t().syp }}</div>
                  </div>
                </div>
            ...
            <div class="flex-1 p-5 sm:p-12 lg:p-16 overflow-y-auto">
              <div class="max-w-2xl mx-auto">
                ...
                <div class="mb-8 sm:mb-12">
                  <h2 class="text-2xl sm:text-4xl font-black mb-2 sm:mb-4 tracking-tight">{{ t().paymentDetails }}</h2>
                  <p class="text-sm sm:text-lg text-slate-500 font-medium">{{ t().paymentDesc }}</p>
                </div>
                  ...
                  <div class="space-y-8 sm:space-y-10">
                      ...
                      @if (selectedMethod()) {
                        ...
                        <div class="mt-4 sm:mt-6 p-4 sm:p-6 rounded-2xl sm:rounded-3xl bg-slate-100 dark:bg-slate-900/50 border border-slate-200 dark:border-slate-800 animate-[slideUp_0.3s_ease-out]">
                          <div class="flex items-center gap-2 mb-3 sm:mb-4">
                            ...
                            <span class="text-xs sm:text-sm font-black text-slate-600 dark:text-slate-400">{{ t().paymentInstructionsTitle }}</span>
                          </div>
                          ...
                          @if (method?.qrCodeUrl || method?.qrCode || method?.walletAddress || method?.details) {
                            ...
                            <div class="flex flex-col sm:flex-row items-center gap-4 sm:gap-6 p-4 sm:p-6 bg-white dark:bg-slate-800 rounded-xl sm:rounded-2xl shadow-md border border-blue-100 dark:border-blue-900/30">
                              ...
                              <div class="flex-1 text-center sm:text-right">
                                <h4 class="text-lg sm:text-xl font-black text-blue-600 dark:text-blue-400 mb-1 sm:mb-2">{{ t().scanToPay }}</h4>
                                <p class="text-[10px] sm:text-sm text-slate-500 font-bold leading-relaxed">{{ t().scanToPayDesc }}</p>
                              </div>
                            </div>
                          }
                        </div>
                    ...
                    <section>
                      <div class="flex items-center gap-2 sm:gap-3 mb-5 sm:mb-6">
                        ...
                        <h3 class="text-lg sm:text-xl font-black">{{ t().confirmPayment }}</h3>
                      </div>
                          ...
                          <label class="relative flex flex-col items-center justify-center w-full p-6 sm:p-10 border-2 border-dashed rounded-[1.5rem] sm:rounded-[2rem] cursor-pointer hover:bg-blue-50/30 dark:hover:bg-blue-900/5 transition-all group overflow-hidden"
                            ...
                            @if (receiptFile()) {
                              <div class="text-center animate-[scaleUp_0.3s_ease-out]">
                                ...
                                <button (click)="$event.preventDefault(); receiptFile.set(null)" class="mt-3 text-[10px] font-black text-red-500 hover:underline">{{ t().deleteFile }}</button>
                              </div>
                            } @else {
                              <div class="text-center transition-transform group-hover:scale-110">
                                ...
                                <div class="text-base sm:text-lg font-black">{{ t().clickToUpload }}</div>
                                <p class="text-[10px] text-slate-400 font-bold mt-1.5 sm:mt-2">{{ t().uploadReceiptHint }}</p>
                              </div>
                            }
            ...
            <div class="hidden lg:flex w-[400px] bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800 p-12 flex-col">
              <div class="sticky top-12">
                <h3 class="text-xs font-black text-slate-400 uppercase tracking-[0.2em] mb-8">{{ t().orderSummary }}</h3>
                ...
                <div class="p-8 rounded-[2rem] bg-slate-50 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700 mb-8 relative overflow-hidden">
                  ...
                  <div class="flex items-center gap-4 mb-8">
                    ...
                    <div>
                      <div class="text-2xl font-black capitalize">{{ selectedPlanForPayment() }} {{ t().plan }}</div>
                      <div class="text-sm font-bold text-slate-500">{{ billingCycle() === 'monthly' ? t().monthly : t().yearly }}</div>
                    </div>
                  </div>

                  <div class="space-y-4 pt-6 border-t border-slate-200 dark:border-slate-700">
                    <div class="flex justify-between items-center text-sm font-bold">
                      <span class="text-slate-500">{{ t().packagePrice }}</span>
                      <span>{{ getPrice(selectedPlanForPayment()!, 'usd') }}$</span>
                    </div>
                    <div class="flex justify-between items-center text-sm font-bold">
                      <span class="text-slate-500">{{ t().tax }}</span>
                      <span class="text-green-500">0.00$</span>
                    </div>
                    <div class="pt-4 flex justify-between items-end">
                      <div>
                        <div class="text-xs font-black text-slate-400 uppercase mb-1">{{ t().totalAmount }}</div>
                        <div class="text-3xl font-black">{{ getPrice(selectedPlanForPayment()!, 'usd') }}$</div>
                      </div>
                      <div class="text-right">
                        <div class="text-xs font-black text-blue-600 dark:text-blue-400 mb-1">{{ t().inSyp }}</div>
                        <div class="text-xl font-black text-blue-600 dark:text-blue-400">{{ getPrice(selectedPlanForPayment()!, 'syp') }} {{ t().syp }}</div>
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="space-y-6">
                  <h4 class="text-sm font-black text-slate-800 dark:text-white mb-4">{{ t().whatYouGet }}</h4>
                  <ul class="space-y-4">
                ...
                <div class="mt-12 p-6 rounded-2xl bg-blue-50 dark:bg-blue-900/10 border border-blue-100 dark:border-blue-900/30">
                  <div class="flex gap-3">
                    ...
                    <div>
                      <p class="text-sm font-black text-blue-900 dark:text-blue-100 mb-1">{{ t().needHelp }}</p>
                      <p class="text-xs font-bold text-blue-700/70 dark:text-blue-300/70 leading-relaxed">{{ t().supportAvailable }}</p>
                      <button class="mt-3 text-xs font-black text-blue-600 dark:text-blue-400 hover:underline">{{ t().contactUs }}</button>
                    </div>
                  </div>
                </div>
              </div>
...
export class UpgradeModalComponent implements OnInit {
  ...
  async loadData() {
    try {
      // We don't need to set pricing locally anymore, we use authService.pricing()
      // But we can trigger a refresh if needed
      await this.authService.getPricing();

      const methods = await this.authService.getPaymentMethods();
      if (methods.length === 0) {
        this.setFallbackPaymentMethods();
      } else {
  ...
  private setFallbackPaymentMethods() {
    ...
    this.selectedMethod.set('fallback');
  }
  ...
  getPrice(planType: 'pro' | 'premium', currency: 'usd' | 'syp'): number {
    const p = this.authService.pricing();
    
    // Fallback defaults if signal is null or data missing
    const defaults: any = {
    ...
    if (!p || !p[planType]) {
      return defaults[planType][this.billingCycle()][currency] || 0;
    }
    
    const cycle = this.billingCycle();
    if (!p[planType][cycle]) {
      return defaults[planType][cycle][currency] || 0;
    }

    return p[planType][cycle][currency] || 0;
  }
